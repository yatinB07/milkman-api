...
use App\Http\Controllers\Api\V1\Rider\RiderSubscriptionOrderController;
    ...
    Route::prefix('rider')
        ...
        ->group(function (): void {
            ...
            Route::apiResource('pages', RiderPageController::class)
                ->only(['index', 'show']);
            Route::post('orders/{order}/decision', [RiderOrderController::class, 'decide'])
                ->whereNumber('order')
                ->name('orders.decision');
            Route::post('orders/{order}/complete', [RiderOrderController::class, 'complete'])
                ->whereNumber('order')
                ->name('orders.complete');
            Route::apiResource('orders', RiderOrderController::class)
                ->only(['index', 'show']);
            Route::post('subscription-orders/{subscriptionOrder}/decision', [RiderSubscriptionOrderController::class, 'decide'])
                ->whereNumber('subscriptionOrder')
                ->name('subscription-orders.decision');
            Route::post('subscription-orders/{subscriptionOrder}/complete', [RiderSubscriptionOrderController::class, 'complete'])
                ->whereNumber('subscriptionOrder')
                ->name('subscription-orders.complete');
            Route::post('subscription-orders/{subscriptionOrder}/items/{item}/complete-date', [RiderSubscriptionOrderController::class, 'completeDate'])
                ->whereNumber(['subscriptionOrder', 'item'])
                ->name('subscription-orders.items.complete-date');
            Route::apiResource('subscription-orders', RiderSubscriptionOrderController::class)
                ->only(['index', 'show']);
        });
    ...
    Route::prefix('store')
        ...
        ->group(function (): void {
            Route::get('dashboard', [StoreDashboardController::class, 'show'])
                ->name('dashboard.show');
            Route::delete('account', [StoreAccountController::class, 'destroy'])
                ->name('account.destroy');
            ...
            Route::post('subscription-orders/{subscriptionOrder}/decision', [StoreSubscriptionOrderController::class, 'decide'])
                ->whereNumber('subscriptionOrder')
                ->name('subscription-orders.decision');
            Route::post('subscription-orders/{subscriptionOrder}/rider', [StoreSubscriptionOrderController::class, 'assignRider'])
                ->whereNumber('subscriptionOrder')
                ->name('subscription-orders.rider');
            Route::post('subscription-orders/{subscriptionOrder}/complete', [StoreSubscriptionOrderController::class, 'complete'])
                ->whereNumber('subscriptionOrder')
                ->name('subscription-orders.complete');
