...
GET  /api/v1/admin/earning-reports
...
`GET /admin/earning-reports` maps `earningreport.php` to a paginated/searchable per-store report. Each row includes completed normal/subscription sale count, gross sales, cash-on-hand amount, delivery charge total, platform commission, payout total, remaining store amount, and rating summary.
