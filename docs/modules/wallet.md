...
The legacy `user_api/u_wallet_up.php` endpoint both increments `tbl_user.wallet` and writes a `wallet_report` credit row. This admin CRUD manages ledger records directly; customer balance updates, wallet top-ups, and order/subscription wallet deductions run through `WalletService` so balance and ledger records are written together.
...
`POST /customer/wallet/top-ups` credits the authenticated customer's wallet and writes a `Credit` ledger row with the legacy message `Wallet Balance Added!!`. Order and subscription placement use the same service for `Debit` ledger rows when `wallet_amount` is greater than zero. The balance update and ledger insert are executed in `WalletService` inside one database transaction so the two records cannot drift apart.
