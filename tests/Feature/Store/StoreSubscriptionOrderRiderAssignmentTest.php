...
class StoreSubscriptionOrderRiderAssignmentTest extends TestCase
{
    use RefreshDatabase;

    public function test_store_can_assign_own_rider_to_own_subscription_order(): void
    {
        ...
        $this->withToken($token)
            ->postJson('/api/v1/store/subscription-orders/'.$order->getKey().'/rider', [
        ...
        $this->assertDatabaseHas('rider_notifications', [
            'rider_id' => $rider->getKey(),
            'title' => 'Subscription order #'.$order->getKey().' assigned.',
            'message' => 'You have a subscription order assigned to you.',
        ]);
    }

    public function test_store_cannot_assign_another_stores_rider_to_subscription_order(): void
    {
        [$store, $token] = $this->storeToken();
    ...
    public function test_store_cannot_assign_rider_to_another_stores_subscription_order(): void
    {
        [$store, $token] = $this->storeToken();
        ...
        $this->withToken($token)
            ->postJson('/api/v1/store/subscription-orders/'.$otherOrder->getKey().'/rider', [
                'rider_id' => $rider->getKey(),
            ])
            ->assertNotFound()
    ...
    public function test_store_subscription_order_rider_assignment_rejects_other_identity_tokens(): void
    {
        ...
        $this->withToken($token)
            ->postJson('/api/v1/store/subscription-orders/1/rider', [
                'rider_id' => $rider->getKey(),
            ])
            ->assertForbidden()
